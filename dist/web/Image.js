...
class XhrBlobUrlCache {
    ...
    static _cleanupIfCapacityExceeded() {
        ...
        if (Object.keys(XhrBlobUrlCache._cachedXhrBlobUrls).length + 1 > XhrBlobUrlCache._maximumItems) {
            let oldestFreeableKey;
            let oldestFreeableEntry;
...
class Image extends RX.Image {
    constructor(props) {
        ...
        this._onLoad = () => {
            ...
            let imageDOM = ReactDOM.findDOMNode(this.refs['image']);
            if (imageDOM) {
    ...
    _actuallyStartXhrImageFetch(props) {
        ...
        if (window.fetch) {
            ...
            req.onload = () => {
                if (req.status >= 400 || req.status < 600) {
                    ...
                    this._handleXhrBlob(req.response);
                }
            };
            req.onerror = () => {
    ...
    render() {
        ...
        let reactElement = (React.createElement("div", { style: this._getStyles(), onMouseUp: this._onMouseUp, title: this.props.title },
            optionalImg,
            this.props.children));
    ...
    _getStyles() {
        let combinedStyles = Styles_1.default.combine(_styles.defaultContainer, this.props.style);
